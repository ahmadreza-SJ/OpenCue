...
class Layer(with_metaclass(LayerType, object)):
    ...
    def execute(self, frame):
        ...
        for env_k, env_v in list(self.__outline.get_env().items()):
            if not env_v[1]:
        ...
        try:
            for env_k, env_v in list(self.__env.items()):
                logger.info("Setting post-set shot environment var: %s %s",
                            env_k, env_v)
    ...
    def setup_args_override(self):
        ...
        try:
            ...
            for key, value in list(args_override.items()):
    ...
    def check_input(self, frame_set=None):
        ...
        if input is missing.
        """
        for name, inpt in list(self.__input.items()):
    ...
    def check_output(self, frame_set=None):
        ...
        if self.get_arg("nocheck"):
            return
        for name, output in list(self.__output.items()):
    ...
    def set_output_attribute(self, name, value):
        ...
        for output in list(self.__output.values()):
    ...
    def set_input_attribute(self, name, value):
        ...
        for output in list(self.__input.values()):
            output.set_attribute(name, value)
    ...
    def load_outputs(self):
        ...
        if not os.path.exists(os.path.join(self.get_path(), "ol:outputs")):
            return
        for name, output in list(self.get_data("ol:outputs").items()):
    ...
    def __create_output_paths(self):
        ...
        """
        for out in list(self.get_outputs().values()):
